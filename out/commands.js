...
function registerDefaultCommands(ctx) {
    ...
    vscode.commands.registerCommand("nationvsc.openSettings", async (setting, user = true) => {
        await vscode.commands.executeCommand(`workbench.action.${user ? "openSettings" : "openWorkspaceSettings"}`, setting?.trim() || "@ext:tryforge.nationvsc");
    }), 
