...
async function applyDecorations(editor) {
	...
	while ((match = ScanRegex.exec(text))) {
		...
		if (
			!(0, _1.locateCodeBlock)(doc, startPos) ||
			(0, _1.isEscaped)(text, matchIndex) ||
			(0, _1.isIgnored)(text, matchIndex)
		)
			continue;
		const full = match[0];
		...
		for (let i = openIndex + 1; i < closeIndex; i++) {
			...
			const ch = text[i];
			if (ch === "[" && (0, _1.isOpeningBracket)(text, i)) depth++;
...
function registerDecorations(ctx) {
	...
	const updateAll = () => {
		for (const editor of vscode.window.visibleTextEditors) {
			if (!_1.Languages.includes(editor.document.languageId)) continue;
