...
exports.GuidesStorageKey = "nationvsc.guidesCache.v1";
...
async function activate(ctx) {
    ...
    const name = ctx.extension.packageJSON.displayName ?? "NationVSC";
    ...
    ctx.subscriptions.push(watcher, watcher.onDidCreate(reload), watcher.onDidChange(reload), watcher.onDidDelete(reload), vscode.workspace.onDidChangeConfiguration(async (e) => {
        if (e.affectsConfiguration("nationvsc"))
            await reload();
    }));
}
/**
    ...
    const showInformationMessage = async (message, openSettings = true) => {
        ...
        if (action === btnReload) {
            if (openSettings)
                ...
                await vscode.commands.executeCommand("nationvsc.reloadFunctionMetadata");
        }
        else if (action === btnOpenSettings) {
            await vscode.commands.executeCommand("nationvsc.openSettings", "nationvsc.global.enabledWorkspaces");
        }
    };
...
async function fetchFunctions(force = false) {
    ...
    if (failed) {
        const text = `Fetching metadata failed for following ${failed} package${failed === 1 ? "" : "s"}: ` +
            failedFetch.join(", ");
...
async function fetchGuides(force = false) {
    ...
    const url = "https://raw.githubusercontent.com/nationdex/nationvsc/refs/heads/metadata/guides.json";
...
function generateUsage(fn, withTypes = false) {
    ...
    const usage = args?.length
        ? `[${args
            .map((arg) => `${arg.rest ? "..." : ""}${arg.name}${arg.required ? "" : "?"}${withTypes ? `: ${arg.type}` : ""}`)
            .join(";")}]`
        : "";
