...
function getString(node) {
	if (typescript_1.default.isStringLiteral(node)) return node.text;
	if (typescript_1.default.isNoSubstitutionTemplateLiteral(node))
		return node.text;
	return undefined;
}
function getBoolean(node) {
	if (node.kind === typescript_1.default.SyntaxKind.TrueKeyword) return true;
	if (node.kind === typescript_1.default.SyntaxKind.FalseKeyword) return false;
	return undefined;
}
function getEnumName(node) {
	node = unwrapExpression(node);
	if (typescript_1.default.isIdentifier(node)) return node.text;
...
function getArgCall(node) {
	...
	if (
		!typescript_1.default.isIdentifier(access.expression) ||
		access.expression.text !== "Arg"
	)
		return undefined;
	...
	param.name =
		name && typescript_1.default.isStringLiteral(name) ? name.text : undefined;
	const desc = node.arguments[offset + 1];
	param.description =
		desc && typescript_1.default.isStringLiteral(desc)
			? desc.text
			: exports.DefaultParam.description;
	if (mode === "rest") {
...
function getParamObject(node) {
	if (!typescript_1.default.isObjectLiteralExpression(node)) return undefined;
	let param = {};
	for (const prop of node.properties) {
		if (!typescript_1.default.isPropertyAssignment(prop)) continue;
		if (
			!typescript_1.default.isIdentifier(prop.name) &&
			!typescript_1.default.isStringLiteral(prop.name)
		)
			continue;
		const key = typescript_1.default.isIdentifier(prop.name)
			? prop.name.text
			: prop.name.text;
...
function getParams(node) {
	if (!typescript_1.default.isArrayLiteralExpression(node)) return undefined;
	const args = [];
...
function getPropertyKey(name) {
	if (
		typescript_1.default.isIdentifier(name) ||
...
function readMetadata(obj) {
	...
	for (const prop of obj.properties) {
		...
		if (key === "name") {
			...
			if (name) fn.name = name.startsWith("$") ? name : "$" + name;
...
function collectBindings(sf) {
	...
	for (const stmt of sf.statements) {
		if (!typescript_1.default.isVariableStatement(stmt)) continue;
...
function extractCustomFunctions(text, fileName) {
	...
	const sf = typescript_1.default.createSourceFile(
		fileName,
		text,
