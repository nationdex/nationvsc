...
function buildGuideURL(guide) {
    ...
    return (_1.DocsUrl +
        paths +
        (guide.targetType === "none" ? `-${guide.id}` : "") +
        `?p=${guide.packageName}`);
}
function matchesGuide(guide, query) {
    ...
    if (query.subCategory != null &&
        !normalize(guide.subCategory).includes(normalize(query.subCategory)))
        return false;
...
function registerGuidePreview(ctx) {
    ...
    // Preview Guide
    vscode.commands.registerCommand("nationvsc.previewGuide", async (input) => {
        let guide = null;
    ...
    // Open Guide
    vscode.commands.registerCommand("nationvsc.openGuide", async (guide) => {
...
function registerGuidesView(ctx) {
    ...
    // Favorite Guide
    vscode.commands.registerCommand("nationvsc.favoriteGuide", async (node) => {
        if (!node.guide)
            return;
    ...
    // Unfavorite Guide
    vscode.commands.registerCommand("nationvsc.unfavoriteGuide", async (node) => {
        if (!node.guide)
            return;
    ...
    vscode.commands.registerCommand("nationvsc.openGuideExternal", async (node) => {
        if (!node?.guide)
            return;
