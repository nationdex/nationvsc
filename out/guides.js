...
function markdownForGuide(guide) {
	...
	lines.push("");
	lines.push(
		`> **Approved:** \`${formatDate(guide.approvedAt)}\` by \`${guide.approver.username}\`\\`,
	);
	const contributors = guide.contributors.map((x) => `\`${x.username}\``);
	lines.push(
		`> **Contributor${contributors.length === 1 ? "" : "s"}:** ${contributors.join(", ")}`,
	);
	lines.push("");
	lines.push("---");
	lines.push("");
...
function matchesGuide(guide, query) {
	...
	if (query.contributorUsername) {
		...
		const hasContributor = (guide.contributors ?? []).some((x) =>
			normalize(x.username).includes(wanted),
		);
		if (!hasContributor) return false;
	}
	...
	const update = (input) => {
		...
		if (!!activeKey && activeKey && values[activeKey]) {
			...
			suggestionItems = suggestions
				.filter(
					(v) =>
						!activeValue || v.toLowerCase().includes(activeValue.toLowerCase()),
				)
				.map((value) => ({
					label: value,
