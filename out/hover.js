    ...
    ctx.subscriptions.push(vscode.languages.registerHoverProvider(_1.Languages, {
        async provideHover(document, position) {
            ...
            if (operatorRange?.contains(position)) {
                ...
                const afterOk = new RegExp(`^(?:${_1.OperatorChain})[a-zA-Z0-9]`).test(after);
                if (!afterOk)
                    return;
            ...
            while ((match = Regex.exec(line))) {
                ...
                if (position.character >= start && position.character <= end) {
                    ...
                    if (version) {
                        ...
                        if (guide) {
                            const cmd = vscode.Uri.parse(`command:nationvsc.previewGuide?${encodeURIComponent(JSON.stringify([guide.id]))}`);
                            links.push(`[${vscode.l10n.t("Guide")}](${cmd})`);
                        }
                        md.appendMarkdown(`---\n`);
                        md.appendMarkdown(`##### ${pkgName ? `${pkgName} ` : ""}v${version}` +
