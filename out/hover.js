...
function registerHover(ctx) {
	ctx.subscriptions.push(
		vscode.languages.registerHoverProvider(_1.Languages, {
			async provideHover(document, position) {
				const config = (0, _1.getExtensionConfig)();
				if (
					!(0, _1.locateCodeBlock)(document, position) ||
					!config.features.hoverInfo
				)
					return;
				...
				// Operator hover
				const operatorRange = document.getWordRangeAtPosition(
					position,
					/@\[[^\]]?\]|[!#]/,
				);
				if (operatorRange && operatorRange.contains(position)) {
					...
					const offset = document.offsetAt(
						new vscode.Position(position.line, opStart),
					);
					if ((0, _1.isIgnored)(text, offset)) return;
					const dollar = line.lastIndexOf("$", opStart);
					if (dollar === -1 || (0, _1.isEscaped)(line, dollar)) return;
					const between = line.slice(dollar, opStart);
					const prefixOnly = between.slice(1);
					if (
						prefixOnly.length &&
						!new RegExp(`^${_1.OperatorChain}$`).test(prefixOnly)
					)
						return;
					const opEnd = operatorRange.end.character;
					const after = line.slice(opEnd);
					const afterOk = new RegExp(
						String.raw`^(?:${_1.OperatorChain})[a-zA-Z0-9]`,
					).test(after);
					if (!afterOk) return;
				...
				while ((match = Regex.exec(line))) {
					...
					if (position.character >= start && position.character <= end) {
						...
						const md = new vscode.MarkdownString();
						md.appendCodeblock(
							`${brackets || hasBracket ? ((0, _1.generateUsage))(fn) : name}${output ? `: ` + output.join(", ") : ""}\n`,
						);
						...
						if (version) {
							...
							const guide = await (0, _1.findGuide)({
								targetType: "function",
								targetName: name,
							});
							const pkgName =
								guide?.packageName || (0, _1.getPackageName)(source);
							if (pkgName)
								links.push(
									`[${vscode.l10n.t("Documentation")}](https://docs.botforge.org/function/${name}?p=${pkgName})`,
								);
							if (guide) {
