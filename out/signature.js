...
class ForgeSignatureHelpProvider {
	async provideSignatureHelp(document, position) {
		...
		if (
			...
			(0, _1.isIgnored)(document.getText(), document.offsetAt(position))
		)
			return null;
		...
		const match = beforeBracket.match(
			new RegExp(_1.FunctionRegex.source + "$"),
		);
		if (!match) return null;
		const startIndex = beforeBracket.lastIndexOf("$");
		if ((0, _1.isEscaped)(beforeBracket, startIndex)) return null;
