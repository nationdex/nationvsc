		...
		vscode.languages.registerHoverProvider(Languages, {
			async provideHover(document, position) {
				...
				if (operatorRange?.contains(position)) {
					...
					const afterOk = new RegExp(`^(?:${OperatorChain})[a-zA-Z0-9]`).test(after);
					if (!afterOk) return;
				...
				while ((match = Regex.exec(line))) {
					...
					if (position.character >= start && position.character <= end) {
						...
						const md = new vscode.MarkdownString();
						md.appendCodeblock(
							`${brackets || hasBracket ? generateUsage(fn) : name}${output ? `: ${(output as Array<string | number>).join(", ")}` : ""}\n`,
						);
						...
						if (version) {
							...
							if (guide) {
								const cmd = vscode.Uri.parse(
									`command:nationvsc.previewGuide?${encodeURIComponent(JSON.stringify([guide.id]))}`,
								);
